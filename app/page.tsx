...
function Wordmark() {
  return (
    <span className="flex items-center gap-2.5 text-lg font-semibold tracking-[-0.035em] text-zinc-950">
...
function SectionLabel({ number, children }: { number: string; children: React.ReactNode }) {
  return (
    <p className="flex items-center gap-3 text-[11px] font-medium uppercase tracking-[0.16em] text-zinc-500">
  ...
  return (
    <main className="overflow-hidden bg-white text-zinc-950">
      <section className="hero" id="top">
        <header className="relative z-10 mx-auto flex h-16 max-w-6xl items-center justify-between border-b border-zinc-200 px-6 lg:px-8">
          <a href="#top" aria-label="Sunda home"><Wordmark /></a>
          <nav className="hidden items-center gap-7 text-sm text-zinc-500 md:flex" aria-label="Primary navigation">
            <a className="transition hover:text-zinc-950" href="#approach">Approach</a>
            <a className="transition hover:text-zinc-950" href="#focus">Focus</a>
            <a className="transition hover:text-zinc-950" href="#region">Why here</a>
          </nav>
          <a className="inline-flex h-9 items-center gap-2 border border-zinc-300 bg-white px-3.5 text-sm font-medium shadow-sm transition hover:border-blue-600 hover:text-blue-700" href="#contact">
        ...
        <div className="relative z-10 mx-auto grid max-w-6xl items-center gap-16 px-6 py-20 md:py-28 lg:grid-cols-[1.05fr_.8fr] lg:px-8 lg:py-32">
          <div>
            <p className="mb-6 flex items-center gap-2.5 text-[11px] font-medium uppercase tracking-[0.16em] text-zinc-500">
            ...
            <p className="mt-7 max-w-xl text-balance text-lg leading-8 text-zinc-600">
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row">
              ...
              <a className="inline-flex h-12 items-center justify-center gap-8 border border-zinc-300 bg-white px-5 text-sm font-medium text-zinc-700 shadow-sm transition hover:border-zinc-400" href="#approach">
        ...
        <div className="relative z-10 mx-auto grid max-w-6xl border-x border-t border-zinc-200 bg-white/90 sm:grid-cols-2 lg:grid-cols-4">
          {signals.map((signal) => (
            <article className="min-h-28 border-b border-r border-zinc-200 p-5 last:border-r-0 lg:border-b-0" key={signal.number}>
              <span className="float-right font-mono text-[9px] text-zinc-300">{signal.number}</span>
              <h2 className="mt-4 text-sm font-medium">{signal.title}</h2>
              <p className="mt-1 text-xs leading-5 text-zinc-500">{signal.body}</p>
            </article>
          ))}
        </div>
      </section>

      <section className="border-y border-zinc-200 bg-zinc-50 py-24 md:py-32" id="approach">
      ...
      <section className="py-24 md:py-32" id="focus">
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-16 grid border-l border-t border-zinc-200 sm:grid-cols-2">
            {focusAreas.map(({ icon: Icon, ...area }) => (
              <article className="group relative min-h-72 border-b border-r border-zinc-200 p-6 transition hover:bg-zinc-50 sm:p-8" key={area.number}>
                <CardDecorator />
                <div className="flex items-start justify-between">
                  <span className="grid size-9 place-items-center border border-zinc-200 bg-white shadow-sm">
                    <Icon className="size-4 text-blue-600" aria-hidden="true" />
                  </span>
                  <span className="font-mono text-[9px] text-zinc-300">{area.number}</span>
                </div>
                <h3 className="mt-14 text-2xl font-semibold tracking-[-0.035em]">{area.title}</h3>
                <p className="mt-3 max-w-sm text-sm leading-6 text-zinc-500">{area.body}</p>
      ...
      <section className="bg-zinc-50 py-24 md:py-32" id="about">
        <div className="mx-auto max-w-5xl px-6">
          <div className="grid gap-10 md:grid-cols-[1fr_340px] md:items-end">
            <div>
          ...
          <div className="mt-16 grid border-l border-t border-zinc-200 md:grid-cols-3">
            {principles.map(([title, body], index) => (
              <article className="relative flex min-h-64 flex-col border-b border-r border-zinc-200 bg-white p-7" key={title}>
                <CardDecorator />
                <span className="font-mono text-[9px] text-zinc-300">0{index + 1}</span>
      ...
      <section className="border-t border-zinc-200 bg-white py-24 md:py-32" id="contact">
        <div className="mx-auto max-w-5xl px-6">
          <FeatureCard className="bg-white px-6 py-10 sm:px-10 md:px-14 md:py-14">
      ...
      <footer className="border-t border-zinc-200 bg-zinc-50">
        <div className="mx-auto grid max-w-5xl gap-10 px-6 py-12 sm:grid-cols-[1fr_auto] sm:items-center">
          <div>
            ...
            <p className="mt-3 text-sm text-zinc-500">Applied intelligence, built in Southeast Asia.</p>
          </div>
          <div className="flex gap-6 text-sm text-zinc-500">
            <a className="hover:text-zinc-950" href="mailto:hello@sunda.works">Email</a>
            <a className="hover:text-zinc-950" href="#top">Back to top ↑</a>
          </div>
          <p className="border-t border-zinc-200 pt-5 font-mono text-[9px] uppercase tracking-[0.12em] text-zinc-400 sm:col-span-2">
