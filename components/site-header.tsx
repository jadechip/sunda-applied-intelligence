...
export function SiteHeader({ inverse = false }: { inverse?: boolean }) {
  ...
  return (
    ...
    >
      <a href="/" aria-label="GradNorm home"><BrandWordmark inverse={inverse} /></a>
      <nav className="hidden items-center gap-7 text-sm md:flex" aria-label="Primary navigation">
        <a className={`transition ${navClass}`} href="/#approach">For AI teams</a>
        <a className={`transition ${navClass}`} href="/opportunities">Opportunities</a>
        <a className={`transition ${navClass}`} href="/#principles">Principles</a>
      </nav>
